...
struct image_dimensions_t *get_png_dimensions(const char *path){
  ...
  struct image_dimensions_t *d = calloc(1, sizeof(struct image_dimensions_t));

  d->width  = read_uint32(buf);
  d->height = read_uint32(buf + 4);
  return(d);
}
...
static int img_main(int argc, const char **argv) {
  // path required
  if (argc < 2) {
    usage();
  }
  ...
  // sniff bytes
  if (is_gif(buf)) {
    output_gif_dimensions(path);
  }else if (is_png(buf)) {
    output_png_dimensions(path);
  }else if (is_jpeg(buf)) {
    output_jpeg_dimensions(path);
  }
